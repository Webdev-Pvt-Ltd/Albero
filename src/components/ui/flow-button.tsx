...
export function FlowButton({ text = 'Modern Button' }: { text?: string }) {
    return (
        <button className="group relative flex items-center gap-1 overflow-hidden rounded-[100px] border-[1.5px] border-[#fff]/40 bg-transparent px-8 py-3 text-sm font-semibold text-[#fff] cursor-pointer transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)] hover:border-transparent hover:text-white hover:rounded-[12px] active:scale-[0.95]">
            {/* Left arrow (arr-2) */}
            ...
            {/* Right arrow (arr-1) */}
            <ArrowRight className="absolute w-4 h-4 right-4 stroke-[#fff] fill-none z-[9] group-hover:right-[-25%] group-hover:stroke-white transition-all duration-[800ms] ease-[cubic-bezier(0.34,1.56,0.64,1)]" />
        </button>
    )
}
